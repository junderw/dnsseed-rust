...
pub fn scan_node(scan_time: Instant, node: SocketAddr, manual: bool) {
	...
	tokio::spawn(async move {
		...
		let peer_result = Peer::new(node.clone(), &TOR_PROXY, Duration::from_secs(timeout), &PRINTER).await;
		
		if let Ok((write, read)) = peer_result {
			let timeout_stream = TimeoutStream::new_timeout(read, scan_time + Duration::from_secs(DATA_STORE.get_u64(U64Setting::RunTimeout)));
			...
			while let Some(msg) = timeout_stream.next().await {
				...
				match msg {
					Some(NetworkMessage::Version(ver)) => {
						...
						if !DATA_STORE.get_regex(RegexSetting::SubverRegex).is_match(&ver.user_agent) {
		...
		if state_lock.recvd_version && state_lock.recvd_verack && state_lock.recvd_pong &&
			...
			let old_state = DATA_STORE.set_node_state(node, AddressState::Good, state_lock.node_services);
...
fn poll_dnsseeds(bgp_client: Arc<BGPClient>) {
	tokio::spawn(async move {
		...
		for seed in ["seed.bitcoin.sipa.be", "dnsseed.bitcoin.dashjr.org", "seed.bitcoinstats.com", "seed.bitcoin.jonasschnelli.ch", "seed.btc.petertodd.org", "seed.bitcoin.sprovoost.nl", "dnsseed.emzy.de"].iter() {
			new_addrs += DATA_STORE.add_fresh_addrs((*seed, 8333u16).to_socket_addrs().unwrap_or(Vec::new().into_iter()));
			new_addrs += DATA_STORE.add_fresh_addrs((("x9.".to_string() + seed).as_str(), 8333u16).to_socket_addrs().unwrap_or(Vec::new().into_iter()));
		}
		...
		let _ = tokio::join!(
			...
			DATA_STORE.write_dns(bgp_clone)
		);
...
fn make_trusted_conn(trusted_sockaddr: SocketAddr, bgp_client: Arc<BGPClient>) {
	...
	tokio::spawn(async move {
		let peer_result = Peer::new(trusted_sockaddr.clone(), &TOR_PROXY, Duration::from_secs(600), &PRINTER).await;
...
fn main() {
	...
	assert!(TOR_PROXY.set(tor_socks5_sockaddr).is_ok(), "TOR_PROXY already set");
	...
	rt.block_on(async move {
		...
		assert!(PRINTER.set(Printer::new(&DATA_STORE)).is_ok(), "PRINTER already set");

		let bgp_client = BGPClient::new(bgp_peerasn, bgp_sockaddr, Duration::from_secs(300), &PRINTER, bgp_identifier);
