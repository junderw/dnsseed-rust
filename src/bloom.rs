...
impl<T: Hash> RollingBloomFilter<T> {
    ...
    pub fn insert(&self, item: &T, roll_duration: Duration) {
            ...
            || state.inserted_since_last_roll > ROLL_COUNT
        {
            ...
            if state.current_generation == GENERATION_COUNT as u8 + 1 {
                state.current_generation = 1;
            }
            ...
            new_generations[0..GENERATION_COUNT - 2]
                .copy_from_slice(&state.inserted_in_last_generations[1..]);
...
fn test_bloom() {
    ...
    assert_eq!(
        filter.get_element_count(),
        [ROLL_COUNT + 1, ROLL_COUNT + 1, 999]
    );
    for i in 0..2000 {
        assert!(filter.contains(&i));
    }
